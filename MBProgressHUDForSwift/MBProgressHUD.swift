...
class MBProgressHUD: UIView {
    ...
    func show(_ animated: Bool) {
        ...
        if graceTime > 0.0 {
            let newGraceTimer: Timer = Timer(timeInterval: graceTime, target: self, selector: #selector(MBProgressHUD.handleGraceTimer(_:)), userInfo: nil, repeats: false)
    ...
    func hide(_ animated: Bool) {
        ...
        if let showStarted = showStarted, minShowTime > 0.0 {
            ...
            guard interv >= minShowTime else {
                minShowTimer = Timer(timeInterval: minShowTime - interv, target: self, selector: #selector(MBProgressHUD.handleMinShowTimer(_:)), userInfo: nil, repeats: false)
                return
            }
        }
...
//        isFinished = true
    ...
    func showAnimated(_ animated: Bool, whileExecutingBlock block: @escaping ()->()) {
        self.showAnimated(animated, whileExecutingBlock: block, onQueue: DispatchQueue.global(qos: DispatchQoS.QoSClass.default), completionBlock: nil)
    }
    ...
    func showAnimated(_ animated: Bool, whileExecutingBlock block: @escaping ()->(), completionBlock completion: MBProgressHUDCompletionBlock?) {
        self.showAnimated(animated, whileExecutingBlock: block, onQueue: DispatchQueue.global(qos: DispatchQoS.QoSClass.default), completionBlock: completion)
    }
    ...
    fileprivate func registerForNotifications() {
        NotificationCenter.default.addObserver(self, selector: #selector(MBProgressHUD.statusBarOrientationDidChange(_:)),
